...
find "$root_dir/raw" -mindepth 2 -type d -not -path "*/speedup/*" | while read dir; do
        ...
        pushd speedup || exit
        parallel -j 3 ffmpeg-normalize {} -v --progress --auto-lower-loudness-target --keep-lra-above-loudness-range-target -c:a libmp3lame -ext mp3 ::: *.mp3 &&
        popd || exit
        popd || exit
        name="$(echo "$dir" | cut -d'/' -f6-)"
        mkdir -p "$root_dir/archive/$name"
        mkdir -p "$root_dir/ready/$name"
        mv "$dir/speedup/normalized/" "$root_dir/ready/$name"
        mv "$dir" "$root_dir/archive/$name"
done
